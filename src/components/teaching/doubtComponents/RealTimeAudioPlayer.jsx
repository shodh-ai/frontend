...
const RealtimeAudioPlayer = forwardRef(({ topic, doubt, sessionId, onComplete, visualizationData , microphoneStream , onPlayingChange}, ref) => {
  ...
  useEffect(() => {
    ...
    if (!topic || !sessionId) {
      setConnectionStatus('Missing topic or sessionId');
      return;
    }

    if (!sessionInProgressRef.current && doubt.trim() !== "") {
      ...
      if (microphoneStream || !isMicRequired()) {
  ...
  const startSession = async () => {
    try {
      ...
      try {
        ...
        const savedKey = process.env.NEXT_PUBLIC_OPEN_AI_KEY  || localStorage.getItem('openai_api_key');
        ...
        if (microphoneStream) {
          ...
          if (audioTracks.length > 0) {
            pc.addTrack(audioTracks[0], microphoneStream);
            addDebugInfo('Added parent’s microphone track to peer connection');
          } else {
            addDebugInfo('No audio tracks in parent’s microphone stream');
          }
        }
  ...
  useEffect(() => {
    ...
    addDebugInfo(`Microphone stream updated: ${microphoneStream ? 'Active' : 'Inactive'}`);
  }, [microphoneStream]);
  ...
  const stopSession = async () => {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        if (microphoneStreamRef.current && typeof microphoneStreamRef.current.getTracks === 'function') {
          try {
          ...
          microphoneStreamRef.current = null; // Clear after stopping
        } else {
          addDebugInfo('No valid microphone stream to stop or already stopped');
          microphoneStreamRef.current = null; // Ensure it's nullified even if undefined
        }
  ...
  const playTextToSpeech = async (text, timestamps = []) => {
    try {
      ...
      // const apiKey = localStorage.getItem('openai_api_key')|| proce
      const apiKey = process.env.NEXT_PUBLIC_OPEN_AI_KEY;
      // console.log("11111111111111", apiKey);
      if (!apiKey) {
      ...
      audio.onplay = () => {
        ...
        setIsAudioPlaying(true);
        if (onPlayingChange && typeof onPlayingChange === 'function') {
          onPlayingChange(true); // Notify parent when audio starts
        }
  ...
  const updateMicrophoneStream = (newStream) => {
    if (peerConnectionRef.current && newStream) {
      ...
      if (audioTracks.length > 0) {
        ...
        if (audioSender) {
          ...
          addDebugInfo('Updated microphone stream in WebRTC session');
        } else {
          peerConnectionRef.current.addTrack(audioTracks[0], newStream);
          addDebugInfo('Added new microphone track to WebRTC session');
        }
        microphoneStreamRef.current = newStream; // Update local ref
      }
    } else if (peerConnectionRef.current && !newStream) {
      const senders = peerConnectionRef.current.getSenders();
      const audioSender = senders.find(sender => sender.track && sender.track.kind === 'audio');
      if (audioSender && audioSender.track) {
        audioSender.track.enabled = false;
        addDebugInfo('Microphone muted in WebRTC session');
        microphoneStreamRef.current = null; // Clear local ref
      }
    }
  };
...
RealtimeAudioPlayer.displayName = 'RealtimeAudioPlayer';
